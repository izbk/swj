    ...
    <div class="table">
        <div class="crumbs">
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>统计局导出表管理</el-breadcrumb-item>
            </el-breadcrumb>
        </div>
		<!--工具条-->
        <el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
            <el-form :inline="true" :rules="queryRules" ref="queryform" :model="queryParam">
                ...
                <el-form-item label="企业代码">
                    <el-input v-model="queryParam.enterpriseCode" :maxlength=10 auto-complete="off"></el-input>
                </el-form-item>
                ...
                <el-form-item>
                    <el-button type="primary" @click="getRealReportList">查询</el-button>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" @click="handleAdd">新增</el-button>
                </el-form-item>
                <el-form-item>
                    <el-button type="primary" @click="handleBatchDeleteByCon">按条件删除</el-button>
                </el-form-item>
                <el-form-item>
        ...
        <el-table :data="tableData" border style="width: 100%" v-loading.body="loading" @selection-change="selsChange">
            <el-table-column type="selection" width="70"></el-table-column>
             ...
             <el-table-column prop="createTime" width="160" label="数据日期" >
        ...
        <div style="margin:10px" v-if="sum!=null">
            <label>销售额汇总:<h2 style="color:red;display:inline">{{sum.sales}}千元</h2></label> <label>零售额汇总 :<h2 style="color:red;display:inline">{{sum.retailSales}}千元</h2></label>
       </div>  
            ...
            <el-form :model="subform" label-width="90px" :rules="formRules" ref="subform">
                ...
                <el-col :span='12'>
                        <el-form-item label="企业代码" prop="enterpriseCode">
                            <el-input v-model="subform.enterpriseCode" :maxlength=10 auto-complete="off"></el-input>
                        </el-form-item>
                    </el-col>
                <el-col :span='12'>
                        <el-form-item label="企业名称" prop="enterpriseName">
                            <el-input v-model="subform.enterpriseName" :maxlength=30 auto-complete="off"></el-input>
                        </el-form-item>
                    </el-col>
                    <el-col :span='12'>
                ...
                <el-col :span='12'>
                        <el-form-item label="数据日期" prop="createTime">
                            <el-date-picker type="date" v-model="subform.createTime"></el-date-picker>
                        </el-form-item>
                    </el-col>
            </el-row>
            </el-form>
...
import {checkMoneyNull,checkMoney,checkRequire,checkMonth,validNum} from '../../../utils/validator.js'
export default {
    data() {
            return {
                sum:null,
                uploadUrl: base + "/realReports/import",
                exportUrl: base+"/realReports/export",
                templateUrl: base + "/realReports/exportTpl",
                fileLoading:false,
                tableData: [],
                pageIndex: 1,
                pageSize: 9,
        ...
        methods: {
            ...
            getExportUrl(){
                this.exportUrl=base+"/realReports/export?id="+this.queryParam.id+"&enterpriseName="+this.queryParam.enterpriseName+"&year="+this.queryParam.year+"&month="+this.queryParam.month+"&createTime="+this.queryParam.createTime;
            },
            handleAdd(index, row) {
            ...
            handleDelete(index, row) {
                this.$confirm('确认删除该记录吗?', '提示', {
                    ...
                    delRealReport(param).then((res) => {
                        if (res.data.code == '000000') {
                            this.formVisible = false;
                            this.getRealReportList();
            ...
            handleBatchDeleteByCon(index, row) {
                this.$confirm('确认按当前条件删除记录吗?', '提示', {
                    type: 'warning'
                }).then(() => {
                    let param = {
                    ...
                    batchDelRealReport(param).then((res) => {
                        if (res.data.code == '000000') {
                            this.getRealReportList();
                            this.$message.success("删除成功");
                        } else {
            ...
            getRealReportList() {
                this.$refs["queryform"].validate((valid) => {
                    if (valid) {
                        ...
                        getRealReportList(params).then((res) => {
                            this.loading = false;
                            if (res.data.code == '000000') {
                                this.tableData = res.data.data.realReportLt;
                ...
                this.$refs["subform"].validate((valid) => {
                    if (valid) {
                        ...
                        if (param.id != "") {
                            ...
                            createRealReport(param).then((res) => {
                                this.formLoading = false;
                                if (res.data.code == '000000') {
                                    this.$message.success("保存成功");
                                    this.formVisible = false;
